...
void RudderAngle::setBase(bool full) {
  ...
  //Serial.printf("Set base full: %d, rudderAngle offset: %i, rotations: %i\n", full, this->offset_, this->rotations_);
} 
...
void RudderAngle::checkMagnetPresence_() {
  ...
  if ((this->magnet_status_ & B00100000) != 32) {  // detect correctly positioned magnet once on start
    this->getMagnetStatus();
...
int RudderAngle::getMagnetStatus(){
  ...
  //    not present           010000  - dec 16 - hex 10
...
void RudderAngle::printRotation() {
  Serial.printf("compute rotation:  angle %i rotation angle %i  rotations %i last read %i\n", this->angle_, this->rotation_angle_, this->rotations_, this->last_angle_read_);   
  Serial.printf("Magnet Status  %02x \n", this->getMagnetStatus());
}
